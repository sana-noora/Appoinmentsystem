...
public class TimeSlotDAO {
    ...
    public List<TimeSlot> getAvailableSlots() throws SQLException {
        List<TimeSlot> slots = new ArrayList<>();
        String sql = "SELECT * FROM time_slots WHERE available = TRUE AND end_time > NOW() ORDER BY start_time ASC";
        try (PreparedStatement stmt = connection.prepareStatement(sql);
             ResultSet rs = stmt.executeQuery()) {
            while (rs.next()) {
                slots.add(mapResultSetToTimeSlot(rs));
    ...
    public List<TimeSlot> getAvailableSlotsByScheduleId(long scheduleId) throws SQLException {
        List<TimeSlot> slots = new ArrayList<>();
        String sql = "SELECT * FROM time_slots WHERE schedule_id = ? AND available = TRUE AND end_time > NOW() ORDER BY start_time ASC";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
